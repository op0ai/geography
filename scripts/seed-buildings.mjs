/**
 * seed-buildings.mjs — pre-populate the building cache for well-known places.
 *
 ...
 *
 *   node scripts/seed-buildings.mjs
 *   node scripts/seed-buildings.mjs --dry
 */

const BASE = process.env.SEED_BASE ?? 'https://geography-globe.op0.workers.dev'
...
console.log(`\nSeeding ${PLACES.length} places × ${RADII.length} radii → ${BASE}`)
console.log(`One query at a time, ${PAUSE_MS}ms apart. Overpass allows 2 concurrent.\n`)

for (const [name, lat, lon] of PLACES) {
  for (const r of RADII) {
    const url = `${BASE}/api/buildings?lat=${lat}&lon=${lon}&r=${r}`
    if (DRY) {
      console.log(`  would seed  ${name} @ ${r}m`)
      continue
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (source === 'edge' || source === 'r2') cached++
        else ok++
        console.log(
          `  ${(source === 'miss' ? 'fetched' : 'cached ').padEnd(8)} ` +
          `${name.padEnd(20)} ${String(r).padStart(4)}m  ` +
          `${String(n).padStart(5)} buildings  ${String(ms).padStart(6)}ms`,
        )
      } else {
        failed++
        failures.push(`${name} @ ${r}m (HTTP ${res.status})`)
        console.log(`  FAILED   ${name.padEnd(20)} ${String(r).padStart(4)}m  HTTP ${res.status}`)
      }
    } catch (e) {
      failed++
      failures.push(`${name} @ ${r}m (${String(e).slice(0, 40)})`)
      console.log(`  FAILED   ${name.padEnd(20)} ${String(r).padStart(4)}m  ${String(e).slice(0, 40)}`)
    }
